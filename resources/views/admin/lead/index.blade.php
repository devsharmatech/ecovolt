        ...
        <table id="leadsTable" class="lead-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <div style="display: flex; gap: 10px;">
                            @if(auth()->user()->getRoleNames()->first() == 'admin' || auth()->user()->getRoleNames()->first() == 'dealer')
                            <button onclick="quickAssign({{ $lead->id }}, '{{ $lead->lead_code }}', {{ $lead->assigned_employee_id ?? 0 }})" 
                                    style="width: 32px; height: 32px; border-radius: 8px; background: #fef3c7; border:none; display: flex; align-items: center; justify-content: center; color: #d97706;" 
                                    title="Quick Assign Executive">
...
<script>
    const employees = {!! json_encode($employees->map(fn($e) => ['id' => $e->id, 'name' => $e->name . ' (' . ucfirst($e->getRoleNames()->first()) . ')'])) !!};
    
    function quickAssign(leadId, leadCode, currentEmpId) {
        let options = employees.reduce((acc, emp) => {
            acc[emp.id] = emp.name;
            return acc;
        }, {});

        Swal.fire({
            title: 'Quick Assign Lead',
            text: `Assigning Lead ${leadCode} to an Executive`,
            icon: 'info',
            input: 'select',
            inputOptions: {
                '': '-- Select Executive --',
            ...
            preConfirm: (empId) => {
                ...
                return fetch(`{{ url($role . '/leads') }}/${leadId}/quick-assign`, {
                    method: 'POST',
                    headers: {
            ...
            if (result.isConfirmed) {
                Swal.fire({
                    icon: 'success',
                    title: 'Assigned!',
                    text: 'Lead has been successfully reassigned.',
                    timer: 2000,
