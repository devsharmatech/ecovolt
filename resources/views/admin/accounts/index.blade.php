    ...
    <div class="pay-table-card">
        <table class="pay-table" style="width: 100%; border-collapse: collapse;">
            ...
            <tbody>
                ...
                <tr>
                    <td><span style="font-weight: 800; color:#3b82f6;">{{ $pay->project ? $pay->project->project_code : 'PAY-'.str_pad($pay->id, 3, '0', STR_PAD_LEFT) }}</span></td>
                    <td>
                        <div style="font-weight: 800;">{{ $pay->customer_name }}</div>
                        <div style="font-size: .75rem; color:#64748b;">{{ $pay->customer_email }}</div>
                    </td>
                    <td>
                        <div style="font-weight: 700;">{{ $pay->type ?? 'Booking' }}</div>
                        <div style="font-size: .75rem; color:#16a34a;">(Milestone Payment)</div>
                    </td>
                    ...
                    <td>
                        ...
                        @if($pay->payment_status != 'Received')
                        <form action="{{ route(auth()->user()->getRoleNames()->first() . '.accounts.verify', $pay->id) }}" method="POST">
                            @csrf
                            @method('PATCH')
                            <button type="submit" class="btn btn-success btn-sm rounded-3 shadow-sm" style="font-size: .7rem; font-weight: 800;">
                                <i class="fas fa-check-circle me-1"></i> VALIDATE
                            </button>
                        </form>
                        @endif
                        <form action="{{ route(auth()->user()->getRoleNames()->first() . '.accounts.destroy', $pay->id) }}" method="POST" onsubmit="return confirm('Delete record?');">
                            @csrf
                            @method('DELETE')
