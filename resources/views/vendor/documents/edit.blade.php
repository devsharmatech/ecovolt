        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route(auth()->user()->getRoleNames()->first() .'.documents.update', $document) }}" method="POST"
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select @error('document_type') is-invalid @enderror"
                                            ...
                                            <option value="KYC"
                                                {{ old('document_type', $document->document_type) == 'KYC' ? 'selected' : '' }}>
                                                KYC</option>
                                            <option value="Agreement"
                                                {{ old('document_type', $document->document_type) == 'Agreement' ? 'selected' : '' }}>
                                                Agreement</option>
                                            <option value="Certificate"
                                                {{ old('document_type', $document->document_type) == 'Certificate' ? 'selected' : '' }}>
                                                Certificate</option>
                                            <option value="License"
                                                {{ old('document_type', $document->document_type) == 'License' ? 'selected' : '' }}>
                                                License</option>
                                            <option value="Invoice"
                                                {{ old('document_type', $document->document_type) == 'Invoice' ? 'selected' : '' }}>
                                                Invoice</option>
                                            <option value="Contract"
                                                {{ old('document_type', $document->document_type) == 'Contract' ? 'selected' : '' }}>
                                                Contract</option>
                                        </select>
                                        @error('document_type')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select @error('status') is-invalid @enderror" id="status12"
                                            ...
                                            <option value="verified"
                                                {{ old('status', $document->status) == 'verified' ? 'selected' : '' }}>
                                                Verified</option>
                                        </select>
                                        @error('status')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="date"
                                            ...
                                            value="{{ old('uploaded_date', $document->uploaded_date->format('Y-m-d')) }}"
                                            required>
                                        @error('uploaded_date')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                ...
                                <div class="col-md-6" id="verified_date_field"
                                    style="{{ old('status', $document->status) == 'verified' ? '' : 'display: none;' }}">
                                    <div class="mb-3">
                                        ...
                                        <input type="date"
                                            ...
                                            value="{{ old('verified_date', $document->verified_date ? $document->verified_date->format('Y-m-d') : date('Y-m-d')) }}">
                                        @error('verified_date')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            statusSelect.addEventListener('change', toggleVerifiedDate);
            toggleVerifiedDate(); // Initial call
        });
    </script>
@endsection
